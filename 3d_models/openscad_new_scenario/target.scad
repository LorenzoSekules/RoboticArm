...
translate([0, 0, -H_hub/2-0.1])
cylinder(h = 0.02*H_hub, r1 = 1.5*R_hub, r2 = 1.5*R_hub, center = true, $fn=6);
//
//for(i=[1:3])
//    rotate([0,0,(i-1)*120])
...
for (i = [1:1]){
        translate([0.032, 0, -l_hub_chaser-0.1-(i-1)*0.1])
        rotate([0,0,30])
...
for (i = [1:7]){       
        translate([0.5, 0, -l_hub_chaser-0.1-(i-1)*0.07])
        rotate([0,90,0])
...
//        chaser(w_hub = 0.794, h_hub = 0.970, l_hub=l_hub_chaser);      
}